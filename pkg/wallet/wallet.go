...
// Open a specified wallet from disk
...
// Save wallet to disk with the specified filename
func (wallet *Wallet) Save(filename string, force bool) (err error) {
	...
	if _, err2 := os.Stat(filename); err2 == nil && !force {
...
// Sign `data` with the current wallet private key
...
// Verfiy specified `data` and `sig` match the corresponding public-key
...
// Verfiy a payload with a specified public-key
