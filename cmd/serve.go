...
func init() {
	...
	serveCmd.PersistentFlags().String(rpcIP, "127.0.0.1", "exposed IP for communication with RPC peers")
	serveCmd.PersistentFlags().Uint16(rpcPort, 24816, "exposed HTTP port for communication with RPC peers")

	// 224.0.0.1 for all nodes, broadcast/multicast test
	serveCmd.PersistentFlags().String(p2pIP, "127.0.0.1", "exposed IP for communication with P2P peers")
	serveCmd.PersistentFlags().Uint16(p2pPort, 16842, "exposed HTTP port for communication with P2P peers")
